...
export async function PaywallContent(props: {
  searchParams?: Promise<PaywallSearchInput>;
  variant?: "doku-canonical";
}) {
  const sp =
    props.variant === "doku-canonical" ? {} : ((await props.searchParams) ?? {});

  const dokuCanonical = props.variant === "doku-canonical";
  ...
  const payHref = (plan: string) => {
    if (dokuCanonical && (plan === "single" || plan === "journey")) {
      return `/paywall/doku/${plan}`;
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface text-on-surface overflow-x-hidden">
      ...
      <SiteFooter showContact={showContact} />
    </div>
  );
}
