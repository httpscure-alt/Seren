...
export async function GET(req: Request, { params }: { params: Promise<Params> }) {
  ...
  try {
              ...
              <div>
                ...
                <div style={{ marginTop: "26px", display: "flex", gap: "18px" }}>
                  {[
                    ...
                    { label: "BARRIER", value: "6.4" },
                    { label: "INFLAMM.", value: "4.9" },
                  ].map((m) => (
                    <div
                      key={m.label}
                      style={{
                    ...
                    >
                      <div style={{ fontSize: "14px", opacity: 0.55, letterSpacing: "2px" }}>
                        {m.label}
                      </div>
                      <div style={{ marginTop: "10px", fontSize: "34px" }}>{m.value}</div>
                    </div>
                  ))}
                </div>
    ...
    console.error("[og] share card render failed", e);
    return new ImageResponse(
      (
        <div
          style={{
